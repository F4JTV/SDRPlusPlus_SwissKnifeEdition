...
// #include <config.h>  (removed: collides with SDR++ core config.h; unused by the MT63 engine)
...
MT63tx::MT63tx()
{
	TxVect = NULL;
	dspPhaseCorr = NULL;
	// Define all scalar members up front: the modulator path reads several of
	// these, and leaving them indeterminate makes a freshly-constructed
	// transmitter's first symbols depend on stack/heap residue.
	WindowLen = 0;
	DataCarriers = FirstDataCarr = 0;
	AliasFilterLen = DecimateRatio = 0;
	InterleavePattern = NULL;
	TxAmpl = 0.0;
	CarrMarkCode = 0;
	CarrMarkAmpl = 0;
	DataInterleave = 0;
...
// values used to computer the blackman3 passband filter shape
	...
	if (FFT.Preset(WindowLen))
		goto Error;
	// FFT.Size is only valid after FFT.Preset(); recompute the carrier-phase
	// mask now (the assignment near the top of this function ran before Preset,
	// when FFT.Size was still indeterminate).
	mask = FFT.Size - 1;
