 ...
 *    Copyright (c) 2007-2019 Dave Freese, W1HKJ
 *
...
template <class type>inline int dspRedspAllocArray(type **Array, int Size) {
	...
	*Array = (type*) malloc(Size * sizeof(type));
	if (*Array == 0) return 1;
	// Zero freshly-allocated buffers: several engine blocks read from history /
	// ring buffers during warm-up before they have been filled, which otherwise
	// reads indeterminate memory and makes sync acquisition non-deterministic.
	// (malloc/free is used by every engine destructor, so allocate to match.)
...
template <class type> int dspAllocArray2D(type ***Array, int Size1, int Size2) {
	int i;
	(*Array) = new type *[Size1 * (sizeof(type *))];
	if ((*Array) == NULL)
		return 1;
	for (i = 0; i < Size1; i++)
		(*Array)[i] = NULL;
	for (i = 0; i < Size1; i++) {
		(*Array)[i] = new type [Size2 * sizeof(type)];
		if ((*Array)[i] == NULL)
			goto Error;
		memset((*Array)[i], 0, Size2 * sizeof(type));
	}
	return 0;
Error:
	for (i = 0; i < Size1; i++)
...
{ 
	if (ReqSpace <= Space) 
		return 0;
	// Grow: allocate a new (correctly sized) buffer, zero it, and carry over the
	// previously valid contents. The original code neither preserved the old
	// samples nor freed the old buffer on growth, which both leaked memory and
	// left the carried-over history region indeterminate -> non-deterministic
	// warm-up in the I/Q split / sync stages. malloc/free is used throughout the
	// engine, so keep this consistent with the destructor.
	type *nw = (type*) malloc(ReqSpace * sizeof(type));
	if (nw == NULL)
		return -1;            // keep the existing buffer intact on failure
	memset(nw, 0, ReqSpace * sizeof(type));
	if (Data) {
		memcpy(nw, Data, Space * sizeof(type));
		free(Data);
	}
	Data = nw;
	Space = ReqSpace; 
...
template <class type> dspFIFO<type>::dspFIFO() {
	Buff = NULL;
}
...
template <class type> void dspFIFO<type>::Free(void) { 
	delete [] Buff;
	Buff = NULL;
}
...
template <class type> int dspFIFO<type>::Preset(int Max) {
	Size = Max + 1;
	if (dspRedspAllocArray(&Buff, Size))
		return -1;
	Rd = 0;
	Wr = 0;
	return 0;
...
template <class type> void dspFIFO<type>::Clear(void) {
	Rd = Wr;
}
...
inline double dspPower(double X) {
	return X * X;
}

inline double dspPower(double I, double Q) {
	return I * I + Q * Q;
}

inline double dspPower(dspCmpx X) {
...
inline double dspPower(double_buff *buff) {
	return dspPower(buff->Data, buff->Len);
}

inline double dspPower(dspCmpx_buff *buff) {
	return dspPower(buff->Data, buff->Len);
}

// dspAmplitude calculations

inline double dspAmpl(double I, double Q) {
	return sqrt(I * I + Q * Q);
}

inline double dspAmpl(dspCmpx X) {
	return sqrt(X.re * X.re + X.im * X.im);
}

// dspPhase calculation (output = <-PI..PI) )

inline double dspPhase(double I, double Q) {
	return atan2(Q,I);
}

inline double dspPhase(dspCmpx X) {
...
inline double dspRMS(double *Data, int Len) {
	return sqrt(dspPower(Data, Len) / Len);
}

inline double dspRMS(dspCmpx *Data, int Len) {
	return sqrt(dspPower(Data, Len) / Len);
}

inline double dspRMS(double_buff *Input) {
	return dspRMS(Input->Data, Input->Len);
}

inline double dspRMS(dspCmpx_buff *Input) {
	return dspRMS(Input->Data, Input->Len);
}
...
void dspWhiteNoise(type &X) {
	...
	Rand = ((double)rand() + 1.0) / ((double)RAND_MAX + 1.0);
	dspPower = sqrt(-2*log(Rand));
	Rand = (double)rand() / (double)RAND_MAX;
	dspPhase = 2 * M_PI * Rand;
...
void CdspcmpxdspWhiteNoise(Cdspcmpx<type> &X) {
	double Rand, dspPower, dspPhase;
	Rand = ((double)rand() + 1.0) / ((double)RAND_MAX + 1.0);
	dspPower = sqrt(-log(Rand));
	Rand = (double)rand() / (double)RAND_MAX;
	dspPhase = 2 * M_PI* Rand;
	X.re = dspPower * cos(dspPhase);
	X.im = dspPower * sin(dspPhase);
}
...
inline int  dspConvdoubleTodspS16(double_buff *dbl, dspS16_buff *dspS16, double Gain = 32768.0) {
	int err = dspS16->EnsureSpace(dbl->Len);
	if (err)
		return -1;
...
struct dspLowPass2elem {
	double Mid, Out;
};

struct dspLowPass2weight {
	double W1, W2, W5;
};
...
inline void dspLowPass2Coeff(typeLen IntegLen, dspLowPass2weight &Weight) {
	Weight.W1 = 1.0 / IntegLen;
	Weight.W2 = 2.0 / IntegLen;
	Weight.W5 = 5.0 / IntegLen;
}
...
template <class type>
dspDelayLine<type>::dspDelayLine() {
	Line = NULL;
}
...
template <class type>
void dspDelayLine<type>::Free(void) {
	free(Line);
	Line = NULL;
}
...
int dspDelayLine<type>::Preset(int MaxdspDelay, int MaxSize) {
	LineSize = MaxSize;
	if (LineSize < (2 * MaxdspDelay))
		LineSize = 2 * MaxdspDelay;
	DataLen = MaxdspDelay;
	dspDelay = MaxdspDelay;
	if (dspRedspAllocArray(&Line, LineSize))
		return -1;
...
int dspDelayLine<type>::Process(type *Inp, int Len) {
	if ((DataLen + Len) > LineSize) {
		dspMoveArray(Line, Line + DataLen - dspDelay, dspDelay);
		DataLen = dspDelay;
	}
	if ((DataLen + Len) > LineSize)
		return -1;
...
public:
	...
	int Preset(
			int FilterLen,
			double *FilterShape_I,
			double *FilterShape_Q,
			int DecimateRate);
	int ComputeShape(double LowOmega, double UppOmega, double (*Window)(double));
//   int Process(double *Inp, int InpLen,
