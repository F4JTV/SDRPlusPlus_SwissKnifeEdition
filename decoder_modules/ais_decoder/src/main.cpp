...
struct Contact {
    ...
    uint64_t firstSeenSeq = 0; // arrival order (assigned once, never changes)
};
...
public:
    AISDecoderModule(std::string name) {
        ...
        // Restore config (host/port + window visibility). The TCP "Enable" state
        // is INTENTIONALLY not restored: the TCP client must never connect on
        // its own at SDR++ startup or plugin load. The user has to tick
        // "Enable TCP" explicitly to bring the socket up. This avoids surprise
        // network activity from the constructor.
        config.acquire();
        ...
        config.release();
        tcpEnabled = false;   // always start with TCP off, regardless of saved state
        ...
        // TCP output: pre-configure host/port but DO NOT open the socket here.
        // The worker is started only when the user ticks "Enable TCP" in the UI.
...
private:
    ...
    void onMessage(const ais::Message& msg) {
        ...
        if (c.firstSeenSeq == 0) { c.firstSeenSeq = ++contactSeqCounter; }
        c.mmsi = msg.mmsi;
        c.lastType = msg.type;
        c.msgCount++;
        c.lastUpdate = now;
    ...
    static void menuHandler(void* ctx) {
        ...
        // --- TCP output settings (work even while the module is disabled) ---
        // UI styled after the ADSB module: a single "Enable TCP" checkbox with
        // an inline status word next to it ("disabled" / "enabled" / "connected").
        ImGui::Separator();
        ...
        if (ImGui::Checkbox(("Enable TCP##ais_tcp_en_" + _this->name).c_str(), &_this->tcpEnabled)) {
            _this->applyTcpConfig();
        }
        ImGui::SameLine();
        if (!_this->tcpEnabled) {
        ...
        else if (_this->sender.isConnected()) {
    ...
    void drawContactsTable() {
        ...
        // Stable order: sort by arrival sequence (first time the MMSI was seen).
        // A new frame updates the existing row in place rather than moving it.
        std::sort(list.begin(), list.end(), [](const Contact* a, const Contact* b) { return a->firstSeenSeq < b->firstSeenSeq; });
    ...
    void applyTcpConfig() {
        sender.configure(hostBuf, port);
        // tcpEnabled fully controls the worker: unchecked tears down the
        // connection and stops the worker thread, checked starts it. We don't
        // persist tcpEnabled — TCP always starts off on launch (see constructor).
        if (tcpEnabled) { sender.start(); }
        else            { sender.stop(); }
        config.acquire();
    ...
    uint64_t contactSeqCounter = 0;   // increments per new MMSI, drives stable row order
};
