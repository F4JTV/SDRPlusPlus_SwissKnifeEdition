...
// ---------------------------------------------------------------------------
//  MFSK Varicode decoder
//
//  Used by FLDIGI for 8PSK, 16PSK, xPSK and PSKR modes (as opposed to the
//  PSK Varicode used by BPSK and QPSK). Reference: FLDIGI psk.cxx line
//  1117-1130, mfskvaricode.cxx table.
//
//  Encoding: each character is a bit string starting with '1' and not
//  containing two consecutive zeros, e.g. 'e' = "1000", 'a' = "10100",
//  ' ' = "100". Characters are separated by "00".
//
//  Decoding: shift bits LSB-first into a shift register. When the trailing
//  3 bits read "001" (separator "00" + start "1" of next char), look up
//  (shreg >> 1) in varidecode[] to find the character. Reset shreg to 1.
//
//  Note: this is the IZ8BLY Varicode (Varicode2), used in MFSK16, THOR,
//  PSKR and the plain PSK 8/16-ary modes.
// ---------------------------------------------------------------------------

namespace fldigi {

    // Full 256-entry lookup table from FLDIGI mfskvaricode.cxx.
    // Each value is the integer representation of the Varicode bit string,
    // with the first bit (always 1) in the most significant position.
    static const unsigned int mfsk_varidecode[256] = {
    ...
    public:
        // Push one bit into the decoder. Returns ASCII char (0..255) or -1.
        // Replicates FLDIGI psk::rx_bit() for _8psk/_16psk/_xpsk/_pskr modes.
        int process(int bit) {
            shreg = (shreg << 1) | (bit ? 1u : 0u);
            // Detect "001" pattern at the trailing 3 bits.
            if ((shreg & 7u) == 1u) {
                unsigned sym = shreg >> 1;
                shreg = 1;
                for (int i = 0; i < 256; i++) {
                    if (mfsk_varidecode[i] == sym) {
                        return i;
                    }
                }
                return -1;
