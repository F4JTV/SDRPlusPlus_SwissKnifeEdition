...
#include <string>
#include "decoder.h"
#include "contestia/decoder.h"

#define CONCAT(a, b) ((std::string(a) + b).c_str())

SDRPP_MOD_INFO{
    /* Name:            */ "contestia_decoder",
    /* Description:     */ "Contestia MFSK decoder (19 submodes, 4..64 tones, 125..2000 Hz, Jalocha/FLDIGI engine)",
    /* Author:          */ "F4JTV",
    /* Version:         */ 0, 1, 0,
    /* Max instances    */ -1
};
...
#define VFO_SAMPLE_RATE  (contestia::CONTESTIA_SR)   // 8 kHz: SSB demod -> 8 kHz audio -> engine
#define VFO_BANDWIDTH    3000.0

class ContestiaDecoderModule : public ModuleManager::Instance {
public:
    ContestiaDecoderModule(std::string name) {
        this->name = name;
        ...
        try {
            ...
            if (c.contains("syncMargin")     && c["syncMargin"].is_number())     { syncMargin     = c["syncMargin"]; }
            if (c.contains("syncInteg")      && c["syncInteg"].is_number())      { syncInteg      = c["syncInteg"]; }
        ...
        modeIdx    = std::clamp(modeIdx, 0, contestia::CONTESTIA_MODE_COUNT - 1);
        snapId     = std::clamp(snapId, 0, snapIntervals.size() - 1);
        syncMargin = std::clamp(syncMargin, 2, 16);
        syncInteg  = std::clamp(syncInteg, 2, 8);
        ...
        decoder = new contestia::ContestiaDecoder(vfo->output);
        decoder->onChar = [this](char c) { onChar(c); };
        decoder->setVFOMode((contestia::VfoMode)vfoMode);
        applyVfoGeometry();
        decoder->setMode(modeIdx);
        decoder->setSyncMargin(syncMargin);
        decoder->setSyncInteg(syncInteg);
...
private:
    ...
    static void menuHandler(void* ctx) {
        ContestiaDecoderModule* _this = (ContestiaDecoderModule*)ctx;
        ...
        if (ImGui::Combo(CONCAT("##contestia_mode_", _this->name), &_this->modeIdx, modesStr())) {
            _this->modeIdx = std::clamp(_this->modeIdx, 0, contestia::CONTESTIA_MODE_COUNT - 1);
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##contestia_vfomode_", _this->name), &_this->vfoMode, "USB\0LSB\0")) {
            _this->vfoMode = std::clamp(_this->vfoMode, 0, 1);
            _this->decoder->setVFOMode((contestia::VfoMode)_this->vfoMode);
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::Combo(CONCAT("##contestia_snap_", _this->name), &_this->snapId, _this->snapIntervals.txt)) {
            if (_this->vfo) { _this->vfo->setSnapInterval(_this->snapIntervals.value(_this->snapId)); }
            _this->saveConfig();
        }

        // --- Band view: audio passband spectrum with the Contestia signal band
        ...
        //     drag to set the signal centre. ----------------------------------
        {
            ...
            ImGui::InvisibleButton(CONCAT("##contestia_band_", _this->name), ImVec2(w, h));
        ...
        ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
        if (ImGui::SliderFloat(CONCAT("##contestia_center_", _this->name), &_this->centerHz,
                               400.0f, 2400.0f, "%.0f Hz")) {
            _this->decoder->setAFFreq(_this->centerHz);
            _this->saveConfig();
        }
        if (ImGui::Button(CONCAT("Auto-center##contestia_ac_", _this->name), ImVec2(menuWidth, 0))) {
        ...
        // --- Sync parameters (advanced) --------------------------------------
        if (ImGui::CollapsingHeader(CONCAT("Sync settings##contestia_synch_", _this->name))) {
            ImGui::LeftLabel("Margin");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
            if (ImGui::SliderInt(CONCAT("##contestia_margin_", _this->name), &_this->syncMargin, 2, 16)) {
                _this->decoder->setSyncMargin(_this->syncMargin);
                _this->saveConfig();
            }
            ImGui::LeftLabel("Integration");
            ImGui::SetNextItemWidth(menuWidth - ImGui::GetCursorPosX());
            if (ImGui::SliderInt(CONCAT("##contestia_integ_", _this->name), &_this->syncInteg, 2, 8)) {
                _this->decoder->setSyncInteg(_this->syncInteg);
                _this->saveConfig();
            }
            if (ImGui::Checkbox(CONCAT("8-bit extended chars##contestia_8bit_", _this->name), &_this->eightBit)) {
        ...
        if (ImGui::Checkbox(CONCAT("Squelch##contestia_sqen_", _this->name), &_this->squelchEnabled)) {
            _this->decoder->setSquelchEnabled(_this->squelchEnabled);
            _this->saveConfig();
        }
        ImGui::BeginDisabled(!_this->squelchEnabled);
        ImGui::SetNextItemWidth(menuWidth);
        if (ImGui::SliderFloat(CONCAT("##contestia_sql_", _this->name), &_this->squelchLevel,
                               contestia::CONTESTIA_SQUELCH_MIN, contestia::CONTESTIA_SQUELCH_MAX, "%.1f dB")) {
        ...
        ImGui::Text("Baud: %5.2f   %d tones / %d Hz",
                    _this->decoder->getBaud(),
                    contestia::CONTESTIA_MODES[_this->modeIdx].tones,
                    contestia::CONTESTIA_MODES[_this->modeIdx].bw);

        // --- Decoded text -----------------------------------------------------
        ImGui::TextUnformatted("Decoded text:");
        {
            std::lock_guard<std::mutex> lck(_this->textMtx);
            ImGui::BeginChild(CONCAT("##contestia_txt_", _this->name), ImVec2(menuWidth, 200.0f), true,
        ...
        if (ImGui::Button(CONCAT("Clear##contestia_clr_", _this->name), ImVec2(menuWidth, 0))) {
            std::lock_guard<std::mutex> lck(_this->textMtx);
    ...
    static const char* modesStr() {
        static std::string s;
        if (s.empty()) {
            for (int i = 0; i < contestia::CONTESTIA_MODE_COUNT; i++) {
                s += contestia::CONTESTIA_MODES[i].name;
                s += '\0';
            }
            s += '\0';
        }
    ...
    void saveConfig() {
        ...
        config.conf[name]["syncMargin"]     = syncMargin;
        config.conf[name]["syncInteg"]      = syncInteg;
    ...
    contestia::ContestiaDecoder* decoder = nullptr;

    OptionList<int, double> snapIntervals;

    // Settings
    int    vfoMode        = contestia::VFO_MODE_USB;
    int    modeIdx        = 11;     // Cont16-500
    int    snapId         = 2;      // 50 Hz
...
MOD_EXPORT void _INIT_() {
    ...
    json def = json({});
    config.setPath(root + "/contestia_decoder_config.json");
    config.load(def);
    config.enableAutoSave();
}

MOD_EXPORT ModuleManager::Instance* _CREATE_INSTANCE_(std::string name) {
    return new ContestiaDecoderModule(name);
}

MOD_EXPORT void _DELETE_INSTANCE_(void* instance) {
    delete (ContestiaDecoderModule*)instance;
}
