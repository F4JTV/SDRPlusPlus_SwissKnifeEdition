...
// Base interface for the Contestia decoder, mirroring the control surface of the
// mfsk_decoder / psk_decoder modules (VFO sideband, AF cursor, mode, squelch,
// start/stop) plus the Contestia-specific sync parameters.
...
namespace contestia {
    ...
    static constexpr float CONTESTIA_SQUELCH_MIN = -100.0f;
    static constexpr float CONTESTIA_SQUELCH_MAX = 0.0f;
    ...
    public:
        ...
        virtual void setMode(int modeIdx) = 0;       // index into CONTESTIA_MODES
        virtual void setAFFreq(double freq) = 0;     // signal CENTRE frequency (Hz)

        virtual void setSyncMargin(int margin) = 0;
        virtual void setSyncInteg(int integ) = 0;
