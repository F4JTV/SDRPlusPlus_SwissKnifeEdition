...
- **System clock synchronization** from the decoded TOW: anchor a precise PC timestamp at each subframe preamble, convert to UTC using the PC clock as a week-number hint, display the live PC ↔ GPS offset, and optionally push the corrected time to the OS real-time clock
...
## Time Sync (setting the PC clock from GPS)

When at least one satellite is being tracked with **C/N0 ≥ 30 dB-Hz** AND a navigation subframe has been successfully decoded, the "Time Sync (GPS UTC)" panel becomes active.

**How it works**

At the instant the preamble of a subframe is detected, the bit synchronizer records a precise PC timestamp. The HOW word of that subframe carries a 17-bit truncated TOW count that, by IS-GPS-200 §20.3.3.2, gives the GPS time of the start of the **next** subframe in Z-counts (1.5 s units, ×4). Subtracting one subframe (6 s, with modulo-week wraparound) yields the GPS TOW at the captured preamble instant. The pair `(PC_time, GPS_TOW)` becomes a time anchor.

For display and clock-setting, the anchor is projected forward to the current PC instant, and the GPS week number is recovered from the PC's own (rough) UTC reading — this works unambiguously as long as the PC clock is within ±3.5 days of correct. The result is converted to UTC by subtracting the leap-second offset (default **18 s**, valid since 2017-01-01; configurable in the GUI in case a new leap second is introduced).

**Accuracy**

The achievable end-to-end accuracy is dominated by the constant DSP-pipeline latency between the antenna and the bit-emission timestamp, typically **10–100 ms** depending on SDR backend, USB latency and CPU load. This is comparable to a well-conditioned NTP link and far better than NTP over a congested or high-latency network. Sub-millisecond accuracy would require explicit pipeline-latency calibration which is not attempted here.

**Permissions**

On **Linux**, `clock_settime(CLOCK_REALTIME, …)` requires the `CAP_SYS_TIME` capability. Either:

```bash
# Grant the capability persistently (preferred):
sudo setcap cap_sys_time+ep $(which sdrpp)
```

…or launch SDR++ with `sudo`. If permission is denied, the panel will show a red message with the exact `setcap` command to run.

On **Windows**, `SetSystemTime` requires running SDR++ as administrator.

**Note on NTP**

If `chrony`, `systemd-timesyncd`, or another NTP daemon is running, it will fight any manual clock change after a few seconds. To use GPS as the authoritative time source, either stop the NTP daemon (`sudo systemctl stop systemd-timesyncd`) or configure it to accept the SHM time source from a separate utility — both are outside the scope of this module.
