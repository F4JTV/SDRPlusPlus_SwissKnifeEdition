...
static int rx_resrc(struct tetra_tmvsap_prim *tmvp, struct tetra_mac_state *tms)
{
	...
	/* Expose the observed air-interface encryption mode on the display
	 * state, so the GUI can show whether the cell is actually using SCK,
	 * CCK or AI-mode 3 on each slot.
	 *
	 * IMPORTANT: only count PDUs that have a real address (i.e. that
	 * actually carry traffic to a user). On many TETRA networks the
	 * signalling / OTAR / management PDUs are CCK-encrypted by default
	 * even when voice traffic is fully in clear — if we counted those
	 * too, the Security panel would permanently report "Encrypted (CCK)"
	 * on a clear cell, which is misleading. By gating on
	 * `addr.type != NULL` we only reflect the encryption status of
	 * actual user-bearing PDUs.
	 *
	 * This is also independent from the KSG (TEA1..TEA7) which is not
	 * broadcast in plaintext SYSINFO. */
	slot = tmvp->u.unitdata.tdma_time.tn;
	if (slot >= 0 && slot < 4 && rsd.addr.type != ADDR_TYPE_NULL) {
		tms->t_display_st->encryption_mode_slot[slot] = rsd.encryption_mode;
		if (rsd.encryption_mode > 0) {
			tms->t_display_st->encryption_mode_last = rsd.encryption_mode;
		}
	}

	/* Per-slot SSI / activity tracking for the UI. Only update when we
	 * have a real address (NULL addresses appear on idle slots and would
	 * just blank the cell). The address carried here is the MAC layer
	 * SSI — for group calls this is the GSSI (group identifier), not
	 * the calling member's ISSI. The CMCE parser overrides this slot
	 * SSI with the actual calling party once a D-SETUP is parsed, which
	 * is what the user expects to see on the panel. */
	if (slot >= 0 && slot < 4 && rsd.addr.type != ADDR_TYPE_NULL) {
		struct timeval _tv;
		gettimeofday(&_tv, NULL);
		/* Only fill the slot if it was previously empty — once the
		 * CMCE parser has set it from a D-SETUP, leave it alone so
		 * the displayed SSI matches the call window's "Caller". */
		if (tms->t_display_st->ssi_slot[slot] == 0) {
			tms->t_display_st->ssi_slot[slot] = (int)rsd.addr.ssi;
		}
		tms->t_display_st->call_state_slot[slot] = 1; /* traffic */
	...
	if (rsd.macpdu_length != MACPDU_LEN_START_FRAG || !REASSEMBLE_FRAGMENTS) {
		 ...
		 * Pass the SDU up to the LLC layer for parsing and routing
		 * to the MLE dispatcher (Session 1 reactivation). */
		tms->curr_active_timeslot = tmvp->u.unitdata.tdma_time.tn;
