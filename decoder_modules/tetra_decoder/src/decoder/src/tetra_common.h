...
/* ===========================================================================
 * Minimal osmocom-style helpers used by the (re-enabled) LLC code.
 *
 * Upstream osmo-tetra relies on:
 *   - libtalloc  for hierarchical memory allocation
 *   - libosmocore linuxlist  for kernel-style circular doubly-linked lists
 *
 * Neither is available in the SDR++ build environment, so we provide tiny
 * stubs here. Memory is just malloc/free (talloc context arguments are
 * ignored), and the linked list is a stripped-down clone of linux/list.h.
 *
 * This is enough for the LLC defragmentation queue, which is the only place
 * those helpers are used. Keep these stubs minimal — if more osmo code is
 * ever pulled in, prefer to wrap it like the existing msgb code instead of
 * extending this header.
 * ========================================================================= */

/* ---- talloc stubs ------------------------------------------------------- */
static inline void *talloc_zero_size(void *ctx, size_t size) {
	(void)ctx;
	return calloc(1, size);
}
#define talloc_zero(ctx, type) ((type *)talloc_zero_size((ctx), sizeof(type)))
static inline void talloc_free_ptr(void *p) {
	if (p) free(p);
}
#define talloc_free(p) talloc_free_ptr(p)

/* ---- linuxlist (intrusive doubly-linked list) --------------------------- */
struct llist_head {
...
static inline void __llist_add(struct llist_head *new_,
...
static inline void llist_add(struct llist_head *new_, struct llist_head *head) {
	__llist_add(new_, head, head->next);
}

static inline void llist_add_tail(struct llist_head *new_, struct llist_head *head) {
	__llist_add(new_, head->prev, head);
}
...
#define llist_for_each_entry_safe(pos, n, head, member)                    \
	for (pos = llist_entry((head)->next, __typeof__(*pos), member),        \
	     ...
	     &pos->member != (head);                                           \
	     pos = n,                                                          \
	     n   = llist_entry(n->member.next, __typeof__(*n), member))
...
struct tetra_display_state {
	...
	bool reg_mandatory;
	/* Air-interface encryption mode actually observed, per timeslot.
	 * 0 = clear, 1 = SCK, 2 = CCK, 3 = CCK + AI mode 3.            */
	uint8_t encryption_mode_slot[4];
	/* Sticky "last non-zero mode observed across any slot".         */
	uint8_t encryption_mode_last;
	/* Last SSI seen on each timeslot (0 if none yet).               */
	int ssi_slot[4];
	/* Coarse call state per slot - Session 1 only tracks "any traffic
	 * seen recently". Sessions 2+ will refine this with CMCE state. */
	uint8_t call_state_slot[4];  /* 0=idle, 1=traffic, 2=ringing, 3=connected */
	/* Activity timestamp (ms since arbitrary epoch). Used by the UI
	 * to fade slots to "idle" if no recent traffic.                 */
	uint64_t ssi_slot_last_ms[4];
	/* Last calling/called SSI extracted from a D-SDS-DATA PDU; used to
	 * thread the addressing information from the CMCE parser down to
	 * the SDS parser without changing every signature.              */
	int last_sds_src;
	int last_sds_dst;
};
