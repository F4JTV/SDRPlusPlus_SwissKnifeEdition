...
// #endif
        ...
        /* Air-interface encryption mode actually observed on a given slot.
         * Returns 0 (clear), 1 (SCK), 2 (CCK) or 3 (CCK + AI mode 3).  */
        int getEncryptionModeSlot(int slot) {
            if (slot < 0 || slot >= 4) return 0;
            return tms->t_display_st->encryption_mode_slot[slot];
        }
        /* Sticky "last non-zero encryption mode seen on any slot".
         * Useful for the summary line in the Security panel.            */
        int getEncryptionModeLast() {
            return tms->t_display_st->encryption_mode_last;
        }

        /* Per-slot SSI / call state tracking, populated by rx_resrc().
         * Sessions 2+ will refine call_state from CMCE D-SETUP/D-CONNECT. */
        int getSsiSlot(int slot) {
            if (slot < 0 || slot >= 4) return 0;
            return tms->t_display_st->ssi_slot[slot];
        }
        int getCallStateSlot(int slot) {
            if (slot < 0 || slot >= 4) return 0;
            return (int)tms->t_display_st->call_state_slot[slot];
        }
        /* Milliseconds since last activity on the slot. Used by the UI
         * to fade slot info if the slot has gone idle. */
        uint64_t getSsiSlotAgeMs(int slot) {
            if (slot < 0 || slot >= 4) return UINT64_MAX;
            uint64_t last = tms->t_display_st->ssi_slot_last_ms[slot];
            if (!last) return UINT64_MAX;
            struct timeval tv;
