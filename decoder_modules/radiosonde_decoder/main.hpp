...
private:
	...
	/* Called by the waterfall when the user resizes the VFO with the mouse.
	 * We then re-tune the FM demodulator and the resampler so the audio path
	 * actually follows the new bandwidth (otherwise the IF filter stays at
	 * the original cutoff). */
	static void onVfoBandwidthChanged(double newBw, void *ctx);
	EventHandler<double> onVfoBandwidthChangedHandler;
};
