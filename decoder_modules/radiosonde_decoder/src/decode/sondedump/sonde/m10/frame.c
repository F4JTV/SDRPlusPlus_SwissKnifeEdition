...
m10_frame_correct(M10Frame *frame)
{
	...
	/* Guard against a corrupted length byte (e.g. when the framer locks onto
	 * noise): an out-of-range len would make crc_ptr point past the frame
	 * buffer and cause an out-of-bounds read. */
	if (frame->len < 1 || frame->len > M10_MAX_DATA_LEN) return -1;

	crc_ptr = (uint8_t*)&frame->len + frame->len - 1;
	expected = crc_ptr[0] << 8 | crc_ptr[1];
