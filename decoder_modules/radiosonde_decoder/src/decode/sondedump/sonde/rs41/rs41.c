...
rs41_decode(RS41Decoder *self, SondeData *dst, const float *src, size_t len)
{
	...
	while (frame_offset < frame_data_len && subframe->len) {
		/* Guard against a corrupted subframe length reading past the frame
		 * buffer (e.g. when locking onto noise). The subframe must fit its
		 * payload plus the trailing 2-byte CRC within the frame storage. */
		const uint8_t *frame_end = (const uint8_t*)self->frame.data + sizeof(self->frame.data);
		if (subframe->data + subframe->len + sizeof(uint16_t) > frame_end) break;

		/* Validate the subframe's checksum against the one received. If it
		 * doesn't match, discard it. The CRC is read byte-wise (little-endian)
		 * to avoid an unaligned 16-bit access. */
		if (crc16_ccitt_false(subframe->data, subframe->len)
		    == (uint16_t)(subframe->data[subframe->len] | (subframe->data[subframe->len + 1] << 8))) {
